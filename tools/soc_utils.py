...
def consistency_loss_soc(logits_w, logits_s, label_dics, clusters, alpha, num_classes):
    ...
    for idx, p in enumerate(pseudo_label):
       ...
       #  class_id = max_idx[idx].cpu().item()
       #
       #  if class_id not in label_dics[conf_idx]:
       #      continue
       #
       #  mapped_id = label_dics[conf_idx][class_id]
       #
       #  if mapped_id not in clusters[conf_idx]:
       #      continue
       #
       #  indices_to_remain = clusters[conf_idx][mapped_id]

        conf_idx = min(num_cluster - 1, int(max_probs_p.item() * num_cluster))
        class_id = max_idx[idx].item()

        if class_id in label_dics[conf_idx]:
            mapped_id = label_dics[conf_idx][class_id]
            if mapped_id < len(clusters[conf_idx]):
                indices_to_remain = clusters[conf_idx][mapped_id]
            else:
                indices_to_remain = [class_id]
        else:
            indices_to_remain = [class_id]  # fallback
